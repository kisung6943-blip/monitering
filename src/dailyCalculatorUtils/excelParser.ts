...
export async function parseExcelOrders(
  ...
  const dateIdx = getColIdx([
    '날짜', '주문일시', '결제일시', '주문일자', '결제일자', '주문일', '결제일',
    '발송일', '일자', '결제완료일', '주문완료일', '정산일', '정산일자', '매출일자', '매출일',
    '정산확정일', '정산기준일', '구매확정일', '구매확정일시', '배송완료일', '배송완료일자'
  ]);
  ...
  const priceIdx = getColIdx(
    platform === 'ohouse'
      ? priceKeywords.filter((k) => !['공급가', '공급가격', '공급금액'].includes(k))
      : priceKeywords,
    ['수수료', '원가']
  );
  const unitPriceIdx = getColIdx(
    ['개별단가', '단가', '옵션+판매가', '옵션+판매', '상품단가', '판매단가', '개별 단가', '상품 단가', '옵션 단가'],
    ['원가', '매입']
  );
  ...
  const settlementIdx = platform === 'ohouse'
    ? getColIdx(
        ['공급가', '공급가격', '공급금액', '정산가(공급가)', '정산금액', '정산예정금액', '정산금액합계', '송금금액', '실정산금액', '정산합계', '정산가'],
        ['수수료', '단가', '배송비']
      )
    : getColIdx(
