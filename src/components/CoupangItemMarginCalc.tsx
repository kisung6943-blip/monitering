...
export default function CoupangItemMarginCalc() {
  ...
  const handleResetSampleData = () => {
    if (confirm("샘플 8개 예시 데이터 상태로 초기화하시겠습니까?")) {
  ...
  return (
    <div className="bg-white text-slate-900 rounded-3xl shadow-xl border border-slate-200 p-4 sm:p-6 lg:p-8 my-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-3">
          <div className="bg-blue-600 text-white p-3 rounded-2xl shadow-md shadow-blue-200 flex items-center justify-center">
            <Calculator size={26} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold px-3.5 py-2 rounded-xl border border-slate-200 transition flex items-center gap-1.5 cursor-pointer shadow-xs"
          >
            <RotateCcw size={14} />
            샘플 데이터 불러오기
          </button>
          <button
            ...
            className="bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold px-3.5 py-2 rounded-xl border border-rose-200 transition flex items-center gap-1.5 cursor-pointer shadow-xs"
          >
            <Trash2 size={14} />
            전체 비우기
          </button>
      ...
      {/* Top Summary Cards (4 Bento Cards - Bright & Modern) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-6">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
          <div>
            <span className="text-xs text-slate-500 font-semibold block">등록 품목 수</span>
        ...
        {/* Card 2 */}
        <div className="bg-emerald-50/60 border border-emerald-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
          <div>
            <span className="text-xs text-emerald-800 font-semibold block">총 예상 순수익 (개당 1회 판매 기준)</span>
        ...
        {/* Card 3 */}
        <div className="bg-blue-50/60 border border-blue-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
          <div>
            <span className="text-xs text-blue-800 font-semibold block">평균 순이익률</span>
        ...
        <div className="bg-amber-50/60 border border-amber-200 rounded-2xl p-4 flex items-center justify-between shadow-xs">
          <div className="max-w-[170px]">
            <span className="text-xs text-amber-800 font-semibold block">최고 마진율 품목</span>
            <span className="text-sm font-extrabold text-amber-900 tracking-tight mt-1 block truncate" title={metrics.highestMarginName}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col gap-4 shadow-xs">
          ...
          {/* Form Mode Selector */}
          <div className="grid grid-cols-2 bg-slate-200/70 p-1 rounded-xl border border-slate-200 text-xs font-bold">
            <button
              type="button"
              onClick={() => setFormMode("retail")}
              className={`py-2 rounded-lg transition ${formMode === "retail" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-900"}`}
            >
              최저가 기준 입력
            </button>
            <button
              ...
              className={`py-2 rounded-lg transition ${formMode === "supply" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-900"}`}
            >
              쿠팡공급가 직접 정하기
            </button>
          </div>
          ...
          <form onSubmit={handleSubmitItem} className="flex flex-col gap-3.5 text-xs">
            {/* 품목명 */}
            <div>
              <label className="block text-slate-700 font-bold mb-1">품목명</label>
              <input
                type="text"
                placeholder="예: 무선 마우스 G-Pro"
                value={inputName}
                onChange={(e) => setInputName(e.target.value)}
                className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-slate-900 placeholder-slate-400 outline-none focus:border-blue-500 transition shadow-xs"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label className="block text-slate-700 font-bold mb-1">제품 원가 (원)</label>
                <input
                  type="number"
                  value={inputCostPrice}
                  onChange={(e) => setInputCostPrice(e.target.value)}
                  className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-slate-900 outline-none focus:border-blue-500 font-mono shadow-xs"
                />
              </div>
              <div>
                <label className="block text-slate-700 font-bold mb-1">포장비 (원)</label>
                <input
                  type="number"
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label className="block text-slate-700 font-bold mb-1">운반비 (센티입고 등)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-slate-700 font-bold mb-1">쿠팡 할인율 (%)</label>
                <select
                  value={inputDiscountRate}
                  onChange={(e) => setInputDiscountRate(Number(e.target.value))}
                  className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-slate-900 outline-none focus:border-blue-500 font-bold shadow-xs"
                >
            ...
            {/* 원가 합계 표시 */}
            <div className="bg-slate-100 border border-slate-200 rounded-xl p-2.5 flex items-center justify-between text-slate-600 font-bold">
              <span>원가 합계 (제품+포장+운반)</span>
              <span className="text-slate-900 font-mono text-sm font-extrabold">{formatKRW(computedCostTotal)}</span>
            </div>

            {/* Form Mode Dependent Fields */}
            {formMode === "retail" ? (
              <div>
                <label className="block text-slate-700 font-bold mb-1">온라인 최저가 (소비자가 입력)</label>
                <input
                  type="number"
                  value={inputRetailPrice}
                  onChange={(e) => setInputRetailPrice(e.target.value)}
                  className="w-full bg-white border border-blue-400 rounded-xl px-3 py-2.5 text-blue-900 font-mono text-sm font-bold outline-none focus:ring-2 focus:ring-blue-200 shadow-xs"
                />
              </div>
            ) : (
              <div>
                <label className="block text-slate-700 font-bold mb-1">쿠팡 공급가 (직접 지정)</label>
                <input
                  type="number"
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center justify-between">
              <span className="text-blue-800 font-bold">계산된 쿠팡 공급가:</span>
              <span className="text-blue-700 font-mono text-base font-black">{formatKRW(computedCalculatedSupplyPrice)}</span>
            </div>

            <button
              type="submit"
              className="mt-2 w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-3 rounded-xl transition shadow-md shadow-blue-200 flex items-center justify-center gap-2 cursor-pointer text-sm"
            >
              <Plus size={16} />
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4 shadow-xs">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 placeholder-slate-400 outline-none focus:border-blue-500 shadow-xs"
              />
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl px-3 py-2 outline-none focus:border-blue-500 shadow-xs"
              >
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead className="bg-slate-100 text-slate-700 sticky top-0 font-bold border-b border-slate-200">
                <tr>
                  ...
                  <th className="p-2.5 text-right text-blue-700">쿠팡공급가</th>
                  ...
                  <th className="p-2.5 text-right text-emerald-700">순수익</th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredAndSortedItems.length === 0 ? (
                  ...
                  filteredAndSortedItems.map((item) => (
                    <tr key={item.id} className="hover:bg-blue-50/40 transition font-mono">
                      <td className="p-2.5 font-sans font-bold text-slate-900 max-w-[150px] truncate" title={item.name}>
                        {item.name}
                      </td>
                      <td className="p-2.5 text-center">
                        <span className="px-1.5 py-0.5 rounded bg-blue-100 text-blue-800 font-bold border border-blue-200 text-[10px]">
                          {item.discountRate}%
                        </span>
                      </td>
